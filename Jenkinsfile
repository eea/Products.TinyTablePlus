pipeline {
  ...
  environment {
        GIT_NAME = "Products.TinyTablePlus"
      ...
      steps {
        parallel(
          ...
          "PloneSaaS": {
            node(label: 'docker') {
              catchError(buildResult: 'SUCCESS', stageResult: 'UNSTABLE') {
                sh '''docker run -i --rm --name="$BUILD_TAG-plonesaas" -e GIT_BRANCH="$BRANCH_NAME" -e ADDONS="$GIT_NAME" -e DEVELOP="src/$GIT_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/plonesaas-devel -v -vv -s $GIT_NAME'''
              }
            }
          }
        )
        ...
        node(label: 'swarm') {
          script{
            ...
            withSonarQubeEnv('Sonarqube') {
                sh "export PATH=$PATH:${scannerHome}/bin:${nodeJS}/bin; sonar-scanner -Dsonar.python.xunit.skipDetails=true -Dsonar.python.xunit.reportPath=xunit-reports/*.xml -Dsonar.python.coverage.reportPath=coverage.xml -Dsonar.sources=./Products -Dsonar.projectKey=$GIT_NAME-$BRANCH_NAME -Dsonar.projectVersion=$BRANCH_NAME-$BUILD_NUMBER"
